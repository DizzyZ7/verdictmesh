...
Проект строится как модульный монолит: сбор данных, evidence collection, аналитические агенты, расчет вероятности, риск-контур, paper trading, аудит и будущий исполнитель разделены на независимые модули, но разворачиваются единым приложением.
...
- GDELT DOC evidence collector для поиска свежих новостных источников;
- дедупликация источников по нормализованному URL;
- authority/freshness scoring и формирование ForecastRequest evidence;
...
- PostgreSQL-аудит рынков, evidence packages, прогнозов, решений, стаканов и виртуальных ордеров;
...
```text
Gamma API + CLOB orderbooks + GDELT DOC + внешние источники
                        ↓
             сбор и нормализация
                        ↓
...
Языковая модель не получает доступ к приватным ключам и не может обойти evidence checks, consensus или risk engine.
...
Контейнер API автоматически применяет Alembic-миграции. При `ORDER_BOOK_SCANNER_ENABLED=true` исторический сбор стаканов начинается автоматически после запуска.

Для реального вызова forecast council добавь `ANTHROPIC_API_KEY` в `.env`. Endpoint `/evidence/collect` и детерминированный `/forecast/consensus` работают без Claude API.
...
`POST /evidence/collect` принимает вопрос рынка, правила resolution, цену YES и опциональные trusted domains. Сервис:

1. строит поисковый запрос;
2. получает article list из GDELT DOC;
3. удаляет дубли по нормализованному URL;
4. оценивает authority, freshness и совпадение с вопросом;
5. формирует evidence package;
6. сохраняет полный пакет в PostgreSQL.

Настройки:

```text
GDELT_API_URL=https://api.gdeltproject.org
EVIDENCE_SEARCH_TIMESPAN=1week
EVIDENCE_SEARCH_MAX_RECORDS=75
EVIDENCE_MAX_ITEMS=8
EVIDENCE_MIN_ITEMS=2
EVIDENCE_QUERY_MAX_TERMS=8
```

`POST /forecast/auto` делает полный путь: evidence collection → ForecastRequest → Claude council. Если источников меньше минимума, forecast не запускается и возвращаются blockers.
...
Текущая цена рынка намеренно не передается агентам, чтобы снизить anchoring. После получения четырех ответов обычный код рассчитывает итоговую вероятность YES, доверительный интервал, disagreement, качество evidence, resolution clarity, raw edge и причины `NO TRADE`.
...
- evidence packages и rejected URLs;
...
1. Автоматический выбор рынков-кандидатов по liquidity, spread, freshness и category.
